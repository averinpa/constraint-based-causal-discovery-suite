"""
bnm v0.2 API sketch — pressure-test before implementing.
...
DESIGN PRINCIPLES (consequences of the bnm 0.1.x audit):
...
2. **One graph contract: `GraphLike`.** A structural Protocol over
   ...
   adaptation; bnm itself imports nothing from cbcd. The
...
4. **Soft dep on pandas; soft dep on viz libs.** The metric layer
   ...
   lazy-imports pandas. Viz lives under `bnm/viz/` and is gated on
...
# =============================================================================
#
# The minimal structural contract every graph passed into bnm must satisfy.
...
# satisfies the Protocol. No inheritance, no import of bnm required.
#
# This is the load-bearing design choice. cbcd's `DAG`, `CPDAG`, `PAG`
# (and similarly any user-defined wrapper) plug into bnm metrics with
# zero conversion overhead; cbcd has no idea bnm exists, and vice
...
class GraphLike(Protocol):
    ...
    `_to_endpoints`), not by bnm metric functions.
    """
...
class EndpointMark(IntEnum):
    ...
    `EndpointMark` so cbcd graphs and bnm graphs are interchangeable
...
# bnm operates on whole graphs, not on individual edges. Decide at
...
# =============================================================================
#
# Every public bnm function accepts `Graph | nx.DiGraph | NDArray |
...
# Type alias for "anything bnm accepts as a graph" — used in the
# signatures below.
...
# `np.meshgrid(*[[0,1]] * 0)` crash (`bnm/sid.py:371` in 0.1.x).


@dataclass(frozen=True, slots=True)
class SIDResult:
    """Output of `bnm.sid`. Includes the full incorrect-intervention
...
# can be passed to any other bnm function. Variable indices are
...
@dataclass(frozen=True, slots=True)
class Comparison:
    """Output of `bnm.compare`. Pure-data; no methods that touch I/O
    or pandas. Use `bnm.to_dataframe(c)` for tabular views.
    """
...
#   matching cbcd's numeric convention. bnm DOES NOT import cbcd; the
...
#   golden snapshot of ~30 random fixtures against bnm 0.1.x output
