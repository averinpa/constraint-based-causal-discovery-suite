...
The `citests` implementation uses random forest regression by default (via the `pycomets` library) for both nuisance regressions and weighting.
...
```python
import numpy as np
from citests.tests import WGCM
...
For a full list of parameters, see the API documentation: :class:`citests.tests.ml_based_tests.WGCM`.
