...
Following Peters & Bühlmann (2015) §3.3, `bnm.sid` returns both
bounds:
...
number of undirected edges. `bnm.sid` uses the Chickering (2002)
...
The number returned by `bnm.sid` is in $\{0, 1, \dots, d(d-1)\}$.
