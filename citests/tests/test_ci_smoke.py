...
from citests.tests.adapter_tests import DiscChiSq, DiscGSq, DummyFisherZ
from citests.tests.nearest_neighbor_tests import MCMIknn
from citests.tests.contingency_table_tests import GSq, ChiSq
...
def test_kci_smoke():
    kernel_module = importlib.import_module("citests.tests.kernel_tests")
...
def test_rcot_smoke():
    ...
    from citests.tests.kernel_tests import RCoT
...
def test_regci_smoke():
    pytest.importorskip("tigramite")
    from citests.tests.regression_tests import RegressionCI
...
def test_hartemink_chisq_smoke():
    pytest.importorskip("rpy2")
    from citests.tests.adapter_tests import HarteminkChiSq
...
def test_cimm_k5_factor_dispatch():
    ...
    from citests.tests.regression_tests import CiMM
    ...
    p_citests = CiMM(data, data_type=data_type)(0, 1, [2])
    ...
    assert p_citests == p_factor, (
        f"CiMM must match direct R factor dispatch bit-identically; "
        f"citests={p_citests!r}, factor={p_factor!r}"
    )
    assert p_citests != p_int, (
        f"CiMM must NOT match the buggy lm/integer path; both gave {p_citests!r}"
    )
    assert p_citests < 0.001  # dependence must still be detected
...
def test_cimm_k2_binary_short_circuit_unchanged():
    ...
    assert np.isfinite(p_citests)
    assert p_citests < 0.01
