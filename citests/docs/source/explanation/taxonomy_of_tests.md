...
- **Examples in `citests`**: `fisherz_citests`, `spearman`.
...
- **Examples in `citests`**: `regci` (tigramite RegressionCI), `ci_mm` (R MXM ci.mm — symmetric, both directions combined).
...
- **Examples in `citests`**: `cmiknn`, `cmiknn_mixed`, `mcmiknn`.
...
- **Examples in `citests`**: `gcm`, `wgcm`, `pcm` (all via `pycomets` with random forest regression by default).
...
- **Examples in `citests`**: `disc_chisq`, `disc_gsq` (equal-frequency discretisation + `chisq`/`gsq`); `dummy_fisherz` (one-hot encoding + Fisher's combined `fisherz`); `hartemink_chisq` (Hartemink information-preserving discretisation via R `bnlearn` + `chisq`).
