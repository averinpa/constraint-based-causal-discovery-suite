"""Internal data normalization + graph summary (shared by the run scaffold).

Lives at the package top level (not under ``cbcd.algorithms``) so the scaffold in ``cbcd._run`` can
import it without triggering the algorithms package init — that would create an import cycle.
"""
...
def _graph_summary(endpoints: NDArray[np.int8], n_vars: int) -> dict[str, int]:
    """A compact recorder ``result_summary``: variable and edge counts. Accuracy metrics (SHD, F1,
    orientation) are computed offline from the returned graph, not here."""
    n = int(n_vars)
    n_edges = sum(
        1 for i in range(n) for j in range(i + 1, n) if endpoints[i, j] != EndpointMark.NO_EDGE
    )
    return {"n_vars": n, "n_edges": int(n_edges)}
