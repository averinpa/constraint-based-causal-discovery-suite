...
`bnmetrics.markov_blanket(g, var)` returns a sub-`GraphLike` restricted
to $\{X\} \cup \mathrm{MB}(X)$, preserving every endpoint mark
incident to those nodes. `bnmetrics.markov_blanket_indices` returns the
indices only.
...
`bnmetrics.plot_graph` accepts the MB subgraph directly. Highlighting
...
`bnmetrics.analyse_mb` produces a multi-panel `plotly` figure summarising
