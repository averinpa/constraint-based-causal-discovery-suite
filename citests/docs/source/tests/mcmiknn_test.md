...
`citests` vendors the upstream `indeptests` package under `citests/_vendor/indeptests/` so the test is available out of the box (see `citests/_vendor/NOTICE.md` for the upstream source URL and the vendored revision SHA).
...
- **Vendored implementation.** No additional installation is required; `citests` ships the upstream `indeptests` package under `citests/_vendor/` (Hügle et al., 2023).
...
- **Dtype validation is opt-in.** Passing data outside the declared dtype produces undefined results; call `Test.validate_data(data)` to check. citests does not enforce ``supported_dtypes`` at construction.

## Code Example

```python
import numpy as np
from citests.tests import MCMIknn
...
For a full list of parameters, see the API documentation: :class:`citests.tests.nearest_neighbor_tests.MCMIknn`.
