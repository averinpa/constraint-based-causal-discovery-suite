...
- [x] **mCMIkNN** — Added wrapper from `vendor/mCMIkNN/src` with lazy local import. Registered as `'mcmiknn'`.
...
- [x] **DCT** — Added wrapper from `vendor/DCT` with lazy local import. Registered as `'dct'`.
...
- [x] **Create `citests/tests/r_based_tests.py`** — rpy2-dependent tests isolated and imported conditionally in `__init__.py`.
