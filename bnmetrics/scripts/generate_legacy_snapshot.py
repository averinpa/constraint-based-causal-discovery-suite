"""Generate tests/fixtures_legacy.json from bnmetrics 0.1.x.

This script is run ONCE on the pre-rewrite bnmetrics 0.1.x source to freeze the
...
PYTHONHASHSEED=0 IS REQUIRED for reproducibility. bnmetrics 0.1.x's SID
...
Notes on importing bnmetrics 0.1.x:
    bnmetrics/__init__.py at HEAD imports `from .viz import ...` which pulls
    ...
    sys.modules BEFORE the bnmetrics import so the script runs in any
    environment with networkx + numpy + pandas (which are already in the
    cbcd venv). The 0.1.x source itself was relocated to
    `scripts/legacy_0_1_x/` once v0.2 took over `bnmetrics/`.
"""
...
# --- stub viz/notebook deps before importing bnmetrics --------------------------
...
# v0.2 took over `bnmetrics/`. We add `scripts/` to sys.path and import the
# legacy package under its new name to keep this generator runnable
# without git checkouts.
sys.path.insert(0, str(SCRIPTS_DIR))

import legacy_0_1_x as bnmetrics  # noqa: E402
...
def build_snapshot() -> dict:
    ...
    return {
        "meta": {
            "bnm_version": bnmetrics.__version__ if hasattr(bnmetrics, "__version__") else "0.1.0",
