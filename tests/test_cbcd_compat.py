...
Verifies that citk's CI test classes satisfy the structural ``cbcd.CITest``
Protocol — no inheritance from a cbcd class, no import of cbcd in citk's
...
def test_citk_fisherz_drives_cbcd_pc():
    """``cbcd.pc()`` accepts a citk FisherZ instance and recovers the
...
def test_citk_fisherz_caches_through_cbcd():
    ...
    automatically), citk's details() return value is what gets cached.
