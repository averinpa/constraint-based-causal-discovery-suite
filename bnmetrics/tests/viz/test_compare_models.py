"""Smoke tests for `bnmetrics.compare_models_descriptive` and
...
def test_descriptive_returns_plotly_figure() -> None:
    ...
    fig = bnmetrics.compare_models_descriptive(graphs, names)
...
def test_descriptive_traces_one_per_metric() -> None:
    ...
    graphs, names = _three_models()
    fig = bnmetrics.compare_models_descriptive(graphs, names)
    n_metrics = len(bnmetrics.DESCRIPTIVE_METRIC_NAMES)
    assert len(fig.data) == n_metrics


def test_descriptive_with_per_node_adds_dropdown() -> None:
    graphs, names = _three_models()
    fig = bnmetrics.compare_models_descriptive(graphs, names, per_node=True)
...
def test_descriptive_specific_metrics() -> None:
    graphs, names = _three_models()
    fig = bnmetrics.compare_models_descriptive(graphs, names, descriptive=["n_edges", "n_colliders"])
...
def test_descriptive_unknown_metric_raises() -> None:
    ...
    with pytest.raises(bnmetrics.BNMInputError, match="unknown descriptive metric"):
        bnmetrics.compare_models_descriptive(graphs, names, descriptive=["not-a-metric"])


def test_descriptive_length_mismatch_raises() -> None:
    graphs, _ = _three_models()
    with pytest.raises(bnmetrics.BNMInputError, match="model_names"):
        bnmetrics.compare_models_descriptive(graphs, ["only_two", "labels"])


def test_descriptive_empty_input_raises() -> None:
    with pytest.raises(bnmetrics.BNMInputError, match="at least one"):
        bnmetrics.compare_models_descriptive([], [])


def test_descriptive_x_axis_uses_model_names() -> None:
    graphs, names = _three_models()
    fig = bnmetrics.compare_models_descriptive(graphs, names, descriptive=["n_edges"])
...
def test_descriptive_save_html(tmp_path) -> None:
    ...
    bnmetrics.compare_models_descriptive(graphs, names, save=out)
    assert out.exists()
...
def test_descriptive_works_on_index_only_graphs() -> None:
    ...
    g2 = make_dag(3, [(0, 1)])
    fig = bnmetrics.compare_models_descriptive(
...
def test_comparative_returns_plotly_heatmap() -> None:
    graphs, names = _three_models()
    fig = bnmetrics.compare_models_comparative(graphs, names, metric="shd")
    assert hasattr(fig, "data")
...
def test_comparative_chain_vs_fork_matches_pairwise_shd() -> None:
    """The off-diagonal entries should equal direct bnmetrics.shd() calls."""
    graphs, names = _three_models()
    fig = bnmetrics.compare_models_comparative(graphs, names, metric="shd")
    z = fig.data[0].z
    # z[j][i] = shd(g1=graphs[i], g2=graphs[j])
    expected_chain_vs_fork = bnmetrics.shd(graphs[0], graphs[1])  # i=0 chain, j=1 fork
    assert z[1][0] == expected_chain_vs_fork


def test_comparative_with_per_node_dropdown() -> None:
    graphs, names = _three_models()
    fig = bnmetrics.compare_models_comparative(graphs, names, metric="shd", per_node=True)
    assert len(fig.layout.updatemenus) == 1
    button_labels = [b.label for b in fig.layout.updatemenus[0].buttons]
    assert "All" in button_labels
    assert "A" in button_labels
...
def test_comparative_unknown_metric_raises() -> None:
    graphs, names = _three_models()
    with pytest.raises(bnmetrics.BNMInputError, match="unknown comparative metric"):
        bnmetrics.compare_models_comparative(graphs, names, metric="not-a-metric")


def test_comparative_length_mismatch_raises() -> None:
    graphs, _ = _three_models()
    with pytest.raises(bnmetrics.BNMInputError, match="model_names"):
        bnmetrics.compare_models_comparative(graphs, ["only_two", "labels"])


def test_comparative_too_few_models_raises() -> None:
    with pytest.raises(bnmetrics.BNMInputError, match="at least two"):
        bnmetrics.compare_models_comparative([chain_3()], ["only"])


def test_comparative_save_html(tmp_path) -> None:
    graphs, names = _three_models()
    out = tmp_path / "comp.html"
    bnmetrics.compare_models_comparative(graphs, names, metric="shd", save=out)
    assert out.exists()


def test_comparative_default_metric_is_shd() -> None:
    graphs, names = _three_models()
    fig = bnmetrics.compare_models_comparative(graphs, names)
...
def test_comparative_works_with_f1() -> None:
    graphs, names = _three_models()
    fig = bnmetrics.compare_models_comparative(graphs, names, metric="f1")
    z = fig.data[0].z
    # Self-pair → F1 = 1.0
    for i in range(3):
        assert z[i][i] == 1.0
...
def test_descriptive_realistic_asia_alpha_sweep() -> None:
    ...
    g_est1 = bnmetrics.to_graphlike(arr_truth, var_names=g_truth.var_names)
    arr2 = arr_truth.copy()
    arr2[0, 1] = 0  # drop asia → tub
    arr2[1, 0] = 0
    g_est2 = bnmetrics.to_graphlike(arr2, var_names=g_truth.var_names)
    arr3 = arr2.copy()
    arr3[2, 3] = 0  # drop smoke → lung
    arr3[3, 2] = 0
    g_est3 = bnmetrics.to_graphlike(arr3, var_names=g_truth.var_names)
...
def test_comparative_realistic_asia_alpha_sweep() -> None:
    ...
    fig = bnmetrics.compare_models_comparative(
