...
## API surface in `bnmetrics`

| call | returns | use |
|---|---|---|
| `bnmetrics.markov_blanket_indices(g, var)` | tuple of int indices for $\{X\} \cup \mathrm{MB}_G(X)$ | feed into a custom membership metric (e.g. Jaccard) |
| `bnmetrics.markov_blanket(g, var)` | sub-`GraphLike` over $\{X\} \cup \mathrm{MB}_G(X)$, with every endpoint mark incident to those vertices preserved | pass to any standard comparative metric for the *structural* error |
| `bnmetrics.compare(g1, g2, per_node=True)` | a `Comparison` whose `.per_node` field maps each vertex name to its descriptive + comparative metrics, computed on the MB sub-graph | survey MB-scoped error across all vertices in one call |
