...
The `citests` implementation uses sample-splitting with random forest regression by default (via the `pycomets` library).
...
For a full list of parameters, see the API documentation: :class:`citests.tests.ml_based_tests.PCM`.
