...
def test_collider_blanket_apex() -> None:
    ...
    g = collider_3()
    indices = bnm.markov_blanket_indices(g, "C")
...
def test_collider_blanket_parent_includes_co_parent() -> None:
    ...
    g = collider_3()
    indices = bnm.markov_blanket_indices(g, "A")
    assert indices == (0, 1, 2)
...
def test_y_blanket_leaf() -> None:
    ...
    g = y_4()
    indices = bnm.markov_blanket_indices(g, "D")
    assert indices == (2, 3)
...
def test_blanket_returns_subgraph_with_renumbered_indices() -> None:
    ...
    sub = bnm.markov_blanket(g, "D")
    assert sub.n_vars == 2
    assert sub.var_names == ("C", "D")
    # In the sub-graph, C is now index 0 and D index 1; the C→D edge survives.
    assert sub.endpoints[0, 1] == bnm.EndpointMark.ARROW
    assert sub.endpoints[1, 0] == bnm.EndpointMark.TAIL
...
def test_blanket_metrics_compose() -> None:
    ...
    g = y_4()
    sub = bnm.markov_blanket(g, "C")  # MB(C) = {A, B, C, D}, all 4 nodes
    assert bnm.count_edges(sub) == 3
    assert bnm.count_colliders(sub) == 1
