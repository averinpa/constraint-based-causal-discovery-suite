...
All tests in `citests` that are compatible with `causal-learn` are designed to be called directly by the search algorithm (like `pc`). The algorithm handles the interpretation internally.
