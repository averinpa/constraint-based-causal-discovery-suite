...
Both delegate the metric computation to :func:`bnm.compare`. Plotly
...
from bnm.viz._sid_matrix import _save_figure
...
def _require_plotly_subplots() -> tuple[Any, Any]:
    try:
        ...
        raise BNMError(
            "bnm.viz multi-model plots require the `viz` extra. Install with "
            "`pip install bnm[viz]` (adds graphviz, plotly, ipython)."
        ) from exc
    ...
    *,
    ...
    Args:
        ...
        descriptive: which descriptive metrics to plot — an iterable
            of names from :data:`bnm.DESCRIPTIVE_METRIC_NAMES`, or
    ...
    *,
    ...
    Args:
        ...
        metric: which comparative metric to display. Default ``"shd"``.
            Must be one of :data:`bnm.COMPARATIVE_METRIC_NAMES`.
