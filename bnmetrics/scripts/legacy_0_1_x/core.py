...
from bnmetrics.utils import mark_and_collapse_bidirected_edges, get_markov_blanket_subgraph
from bnmetrics import metrics as m
...
class BNMetrics:
    ...
    >>> bnmetrics = BNMetrics(G1, G2)

    >>> import numpy as np
    >>> mat1 = np.array([[0, 1], [0, 0]])
    >>> mat2 = np.array([[0, 0], [1, 0]])
    >>> bnmetrics = BNMetrics(mat1, mat2, node_names=["X1", "X2"])
    """
    ...
    def sid(self, nodes='All', output=True):
        ...
        ... ])
        >>> nodes = ['A', 'B', 'C']
        >>> bnmetrics = BNMetrics(G1, G2, node_names=nodes)
        >>> sid_result = bnmetrics.sid(nodes='C', output=True)
        sid: 2
        sid_lower_bound: 2
        sid_upper_bound: 2
        ================================================
