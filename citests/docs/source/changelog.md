...
All notable changes to `citests` are recorded here. The format is based on
[Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and this project
follows [Semantic Versioning](https://semver.org/).

## [Unreleased] (pre-v0.1.0)

The pre-v0.1.0 cycle scopes `citests` to exactly the 19 Paper 1 benchmark tests
...
- Module layout reorganised to match the Paper 0 survey taxonomy. The seven
  ...
  for `citests.tests`; source-grouped modules sit behind them as implementation
  files.
...
- API reference consolidated into a single
  `docs/source/api/citests.rst` page with sections matching the seven taxonomy
...
- Orphan API reference files merged into the main API page:
  `api/citests.tests.rst`, `api/citests.tests.base.rst`.
