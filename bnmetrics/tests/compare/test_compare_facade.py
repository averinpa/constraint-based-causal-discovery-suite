"""Hand-computed checks for `bnmetrics.compare` and `bnmetrics.to_dataframe`."""
...
def test_compare_single_graph_explicit_comparative_list_raises() -> None:
    ...
    g = chain_3()
    with pytest.raises(bnmetrics.BNMInputError, match="g2 is None"):
        bnmetrics.compare(g, comparative=["shd", "f1"])


def test_compare_single_graph_sid_requires_g2() -> None:
    g = chain_3()
    with pytest.raises(bnmetrics.BNMInputError, match="g2 is None"):
        bnmetrics.compare(g, comparative=None, include_sid=True)
...
def test_compare_two_graphs_with_sid() -> None:
    g = chain_3()
    c = bnmetrics.compare(g, g, include_sid=True)
    assert isinstance(c.sid, bnmetrics.SIDResult)
    assert c.sid.sid == 0
...
def test_compare_chain_vs_fork() -> None:
    chain = chain_3()
    fork = fork_3()
    c = bnmetrics.compare(chain, fork)
...
def test_compare_specific_comparative_metrics() -> None:
    g1 = chain_3()
    g2 = fork_3()
    c = bnmetrics.compare(g1, g2, descriptive=None, comparative=["shd", "f1"])
...
def test_compare_unknown_metric_raises() -> None:
    ...
    with pytest.raises(bnmetrics.BNMInputError, match="unknown descriptive metric"):
        bnmetrics.compare(g, descriptive=["not-a-metric"])
...
def test_per_node_true_iterates_all_vars() -> None:
    g = chain_3()
    c = bnmetrics.compare(g, descriptive=["n_edges"], per_node=True)
...
def test_per_node_iterable_subset() -> None:
    g = chain_3()
    c = bnmetrics.compare(g, descriptive=["n_edges"], per_node=["A", "C"])
...
def test_per_node_with_two_graphs_uses_g1_mb_anchor() -> None:
    ...
    c = bnmetrics.compare(g1, g2, per_node=True)
    assert c.per_node is not None
    a_row = c.per_node["A"]
...
def test_per_node_with_sid() -> None:
    g = chain_3()
    c = bnmetrics.compare(g, g, include_sid=True, per_node=True)
    assert c.per_node is not None
    for var in ("A", "B", "C"):
        row = c.per_node[var]
...
def test_to_dataframe_with_per_node() -> None:
    pytest.importorskip("pandas")
    g = chain_3()
    c = bnmetrics.compare(g, g, per_node=True)
    df = bnmetrics.to_dataframe(c)
...
def test_to_dataframe_pandas_missing_raises_bnm_error(monkeypatch) -> None:
    ...
    try:
        with pytest.raises(bnmetrics.BNMError, match="pandas"):
            bnmetrics.to_dataframe(c)
    finally:
