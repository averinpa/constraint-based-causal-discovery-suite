...
directly. They do NOT match bnmetrics 0.1.x in some cases — see audit §8 for
...
def test_collider_truth_vs_self_cpdag() -> None:
    ...
    result = bnmetrics.sid(truth, truth)
    assert result.sid == 0
    assert result.is_tight
...
def test_directed_edge_reversal() -> None:
    ...
    result = bnmetrics.sid(truth, estimate)
    assert result.sid == 2
    assert result.is_tight
...
def test_g1_with_undirected_rejected() -> None:
    ...
    with pytest.raises(bnmetrics.BNMInputError, match="g1 must be a pure DAG"):
        bnmetrics.sid(g1, g2)
...
def test_g2_with_circle_rejected() -> None:
    ...
    arr[0, 1] = bnmetrics.EndpointMark.CIRCLE
    arr[1, 0] = bnmetrics.EndpointMark.TAIL
    g1 = make_dag(2, [(0, 1)], var_names=("A", "B"))
    g2 = bnmetrics.to_graphlike(arr, var_names=("A", "B"))
    with pytest.raises(bnmetrics.BNMInputError, match="g2 must be a DAG or CPDAG"):
        bnmetrics.sid(g1, g2)
...
def test_n_vars_mismatch_rejected() -> None:
    g1 = chain_3()
    g2 = make_dag(4, [(0, 1)])
    with pytest.raises(bnmetrics.BNMInputError, match="must match"):
        bnmetrics.sid(g1, g2)
...
def test_sid_result_is_frozen() -> None:
    ...
    assert isinstance(result, bnmetrics.SIDResult)
...
def test_is_tight_property() -> None:
    truth = chain_3()
    cpdag = make_cpdag(3, [], [(0, 1), (1, 2)], var_names=("A", "B", "C"))
    tight = bnmetrics.sid(truth, truth)
    not_tight = bnmetrics.sid(truth, cpdag)
