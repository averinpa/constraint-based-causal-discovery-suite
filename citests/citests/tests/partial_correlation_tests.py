...
from .base import CITKResult, CITKTest, NO_SPECIFIED_PARAMETERS_MSG

__all__ = ["FisherZ", "Spearman"]
...
class FisherZ(CITKTest):
    ...
    def __init__(self, data: np.ndarray, **kwargs: Any) -> None:
        ...
        self.check_cache_method_consistent("fisherz_citests", NO_SPECIFIED_PARAMETERS_MSG)
        if data.shape[0] < 4:
            raise ValueError(
...
maybe_register("fisherz_citests", FisherZ)
