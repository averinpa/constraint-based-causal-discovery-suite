...
The optimisation: ``bnm.compare`` normalises ``g1`` / ``g2`` to internal
...
import bnm
from bnm import adapter as bnm_adapter
...
def _external_graphlike(g):
    ...
    is what cbcd's `DAG`/`CPDAG`/`PAG` instances look like to bnm.
    """
...
def test_validate_endpoints_called_once_per_external_input() -> None:
    ...
    with patch.object(
        ...
        bnm.compare(g1_ext, g2_ext, per_node=True)
...
def test_validate_endpoints_called_once_for_single_graph_mode() -> None:
    ...
    with patch.object(
        ...
        bnm.compare(g_ext, per_node=True)
...
def test_validate_endpoints_vectorised_on_large_clean_matrix() -> None:
    ...
    tail = int(bnm.EndpointMark.TAIL)
    for i in range(n - 1):
