"""Smoke tests for bnmetrics.viz — verify rendering doesn't crash and the
...
def test_plot_graph_returns_dot_with_chain_edges() -> None:
    g = chain_3()
    dot = bnmetrics.plot_graph(g, title="chain")
    src = dot.source
...
def test_plot_graph_undirected_edge_renders_with_dir_none() -> None:
    ...
    dot = bnmetrics.plot_graph(cpdag)
    src = dot.source
...
def test_plot_graph_highlight_node_filled_pastel_green() -> None:
    g = collider_3()
    dot = bnmetrics.plot_graph(g, highlight=["C"])
    src = dot.source
...
def test_plot_graph_with_int_indices() -> None:
    g = collider_3()
    dot = bnmetrics.plot_graph(g, highlight=[2])
    src = dot.source
    assert "#c8e6c9" in src
...
def test_plot_side_by_side_returns_two_dots() -> None:
    g = chain_3()
    dot1, dot2 = bnmetrics.plot_side_by_side(g, g, name1="A", name2="B")
...
def test_plot_side_by_side_mode_none_skips_edge_highlighting() -> None:
    g = chain_3()
    dot1, dot2 = bnmetrics.plot_side_by_side(g, g, mode="none")
...
def test_plot_side_by_side_highlight_node_in_both() -> None:
    g1 = chain_3()
    g2 = fork_3()
    dot1, dot2 = bnmetrics.plot_side_by_side(g1, g2, highlight_nodes=["A"])
...
def test_plot_side_by_side_n_vars_mismatch_raises() -> None:
    g1 = chain_3()
    g2 = asia_8()
    with pytest.raises(bnmetrics.BNMDataError, match="variables"):
        bnmetrics.plot_side_by_side(g1, g2)


def test_plot_sid_matrix_returns_plotly_figure() -> None:
    result = bnmetrics.sid(chain_3(), chain_3())
    fig = bnmetrics.plot_sid_matrix(result)
...
def test_plot_sid_matrix_uses_var_names() -> None:
    result = bnmetrics.sid(chain_3(), chain_3())
    fig = bnmetrics.plot_sid_matrix(result, var_names=("X", "Y", "Z"))
...
def test_plot_sid_matrix_title_default() -> None:
    truth = chain_3()
    cpdag = make_cpdag(3, [], [(0, 1), (1, 2)], var_names=("A", "B", "C"))
    result = bnmetrics.sid(truth, cpdag)
    fig = bnmetrics.plot_sid_matrix(result)
...
def test_bidirected_edge_renders_with_dir_both() -> None:
    ...
    g = bnmetrics.to_graphlike(arr, var_names=("A", "B"))
    dot = bnmetrics.plot_graph(g)
    src = dot.source
...
def test_subtle_style_explicit() -> None:
    g = chain_3()
    dot = bnmetrics.plot_graph(g, style="subtle")
    assert "shape=ellipse" in dot.source


def test_unknown_style_raises() -> None:
    g = chain_3()
    with pytest.raises(bnmetrics.BNMInputError, match="unknown viz style"):
        bnmetrics.plot_graph(g, style="not-a-style")  # type: ignore[arg-type]


def test_subtle_style_in_side_by_side() -> None:
    g = chain_3()
    dot1, dot2 = bnmetrics.plot_side_by_side(g, g, style="subtle")
...
def test_direction_LR_explicit() -> None:
    g = chain_3()
    dot = bnmetrics.plot_graph(g, direction="LR")
...
def test_direction_auto_picks_LR_for_chain() -> None:
    ...
    g = chain_3()
    dot = bnmetrics.plot_graph(g, direction="auto")
    assert "rankdir=LR" in dot.source
...
def test_direction_invalid_raises() -> None:
    g = chain_3()
    with pytest.raises(bnmetrics.BNMInputError, match="unknown viz direction"):
        bnmetrics.plot_graph(g, direction="diagonal")  # type: ignore[arg-type]


def test_direction_propagates_to_side_by_side() -> None:
    g = chain_3()
    dot1, dot2 = bnmetrics.plot_side_by_side(g, g, direction="LR")
...
def test_plot_graph_save_svg(tmp_path) -> None:
    out = tmp_path / "g.svg"
    g = chain_3()
    dot = bnmetrics.plot_graph(g, save=out)
    assert out.exists()
...
def test_plot_graph_save_png(tmp_path) -> None:
    out = tmp_path / "g.png"
    bnmetrics.plot_graph(chain_3(), save=out)
    assert out.exists()
    # PNG magic bytes.
...
def test_plot_graph_save_no_extension_raises(tmp_path) -> None:
    ...
    with pytest.raises(bnmetrics.BNMInputError, match="no extension"):
        bnmetrics.plot_graph(chain_3(), save=out)


def test_plot_graph_save_accepts_string_path(tmp_path) -> None:
    out = tmp_path / "g.svg"
    bnmetrics.plot_graph(chain_3(), save=str(out))
    assert out.exists()
...
def test_side_by_side_save_single_path_derives_two_files(tmp_path) -> None:
    ...
    g = chain_3()
    bnmetrics.plot_side_by_side(g, g, name1="truth", name2="estimate", save=out)
...
def test_side_by_side_save_tuple_uses_explicit_paths(tmp_path) -> None:
    ...
    bnmetrics.plot_side_by_side(chain_3(), chain_3(), save=(p1, p2))
    assert p1.exists()
    assert p2.exists()
...
def test_side_by_side_save_tuple_wrong_length_raises(tmp_path) -> None:
    p1 = tmp_path / "x.svg"
    with pytest.raises(bnmetrics.BNMInputError, match="exactly two entries"):
        bnmetrics.plot_side_by_side(chain_3(), chain_3(), save=(p1,))
...
def test_sid_matrix_save_html(tmp_path) -> None:
    ...
    result = bnmetrics.sid(chain_3(), chain_3())
    bnmetrics.plot_sid_matrix(result, save=out)
    assert out.exists()
    content = out.read_text()
...
def test_sid_matrix_save_static_image_requires_kaleido(tmp_path) -> None:
    ...
    if kaleido_installed:
        ...
        with pytest.raises(bnmetrics.BNMError, match="kaleido"):
            bnmetrics.plot_sid_matrix(result, save=out)
...
def _pag_three_node_fixture():
    ...
    arr[0, 1] = bnmetrics.EndpointMark.ARROW
    arr[1, 0] = bnmetrics.EndpointMark.CIRCLE
    # B o-o C  ⇒ both ends CIRCLE
    arr[1, 2] = bnmetrics.EndpointMark.CIRCLE
    arr[2, 1] = bnmetrics.EndpointMark.CIRCLE
    return bnmetrics.to_graphlike(arr, var_names=("A", "B", "C"))
