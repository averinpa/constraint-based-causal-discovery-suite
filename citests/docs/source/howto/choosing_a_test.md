...
Choosing the right conditional independence (CI) test depends on the type of your data and the assumptions you are willing to make about the underlying dependence. `citests` ships 19 tests organised under the six survey families plus four adapter strategies; this guide gives a practical mapping from data + assumptions to tests.
...
- **All continuous**:
    - `fisherz_citests`: linear, Gaussian — fastest baseline.
...
- **Linear**: `fisherz_citests` is the computationally efficient choice when both Gaussianity and linearity hold.
...
- **Small samples**: classical tests (`fisherz_citests`, `spearman`, `chisq`, `gsq`) are most reliable; non-parametric and ML-based tests need more data for stable estimation.
...
|-----------|--------|-----------|-------------------|-------------------|
| `fisherz_citests` | Partial Correlation | Continuous | Linear | Approximate Gaussianity |
