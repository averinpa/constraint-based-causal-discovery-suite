...
two graphs using `bnmetrics`, from constructing graph inputs through
...
Note that `bnmetrics` does not import `cbcd` or `dagsampler`. Both
`true_cpdag` and `recovered` are instances of `cbcd.CPDAG`, which
satisfies the structural `bnmetrics.GraphLike` Protocol; `bnmetrics`'s
...
```python
bnmetrics.shd(true_cpdag, recovered)        # 0
bnmetrics.hd(true_cpdag, recovered)         # 0  — skeleton-only
bnmetrics.f1(true_cpdag, recovered)         # 1.0
bnmetrics.precision(true_cpdag, recovered)  # 1.0
bnmetrics.recall(true_cpdag, recovered)     # 1.0
```
...
reversals) is exposed by `bnmetrics.count_additions`,
`bnmetrics.count_deletions`, `bnmetrics.count_reversals`. The full multi-
metric comparison `bnmetrics.compare` runs every comparative metric and
...
```python
sid_result = bnmetrics.sid(true_cpdag, recovered)
print(sid_result)
```

When the recovered graph is a CPDAG rather than a DAG, the SID is
no longer a single integer; `bnmetrics.sid` returns both lower and upper
