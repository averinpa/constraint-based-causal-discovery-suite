...
def test_plot_graph_highlight_node_color_kwarg_overrides_default() -> None:
    g = collider_3()
    dot = bnmetrics.plot_graph(g, highlight=["C"], highlight_node_color="#ff8800")
    src = dot.source
...
def test_plot_side_by_side_highlight_edge_color_kwarg_overrides_default() -> None:
    g = chain_3()
    dot1, dot2 = bnmetrics.plot_side_by_side(
...
def _pag_two_node(mij: int, mji: int):
    ...
    arr[0, 1] = mij
    arr[1, 0] = mji
    return bnmetrics.to_graphlike(arr, var_names=("A", "B"))
...
def test_circle_arrow_does_not_match_circle_circle() -> None:
    ...
    circle = int(bnmetrics.EndpointMark.CIRCLE)
    ...
    dot1, dot2 = bnmetrics.plot_side_by_side(g1, g2, mode="matches")
...
def test_mode_diff_highlights_reversed_edge() -> None:
    ...
    dot1, dot2 = bnmetrics.plot_side_by_side(g1, g2, mode="diff")
    assert "#f08080" in dot1.source
    assert "#f08080" in dot2.source
...
def test_mode_diff_no_highlight_when_g1_equals_g2() -> None:
    ...
    highlighted."""
    g = chain_3()
    dot1, dot2 = bnmetrics.plot_side_by_side(g, g, mode="diff")
    assert "#f08080" not in dot1.source
    assert "#f08080" not in dot2.source
...
def test_mode_matches_default_unchanged() -> None:
    ...
    g = chain_3()
    dot1, dot2 = bnmetrics.plot_side_by_side(g, g)
    assert "#f08080" in dot1.source
    assert "#f08080" in dot2.source
...
def test_mode_invalid_raises() -> None:
    g = chain_3()
    with pytest.raises(bnmetrics.BNMInputError, match="unknown mode"):
        bnmetrics.plot_side_by_side(g, g, mode="bogus")  # type: ignore[arg-type]
