...
The fastest path to a full comparison is `bnm.compare`, which runs
...
bnm.precision(true_cpdag, recovered)         # 0.5
bnm.recall(true_cpdag, recovered)            # 0.5
bnm.count_reversals(true_cpdag, recovered)   # 2
```
...
```python
comp = bnm.compare(true_cpdag, recovered)
