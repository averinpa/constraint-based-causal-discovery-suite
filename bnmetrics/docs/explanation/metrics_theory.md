...
metrics implemented in `bnmetrics`. The treatment follows Tsamardinos,
...
The decomposition is exposed by `bnmetrics.count_additions`,
`bnmetrics.count_deletions`, `bnmetrics.count_reversals`. The aggregated SHD
is `bnmetrics.shd`.
...
de Jongh & Druzdzel (2009) discuss alternative conventions; bnmetrics
...
`bnmetrics.precision`, `bnmetrics.recall`, `bnmetrics.f1`.

```{admonition} On the strictness of TP
:class: warning
`bnmetrics` defines a true positive as an *exact-mark* match — same
...
separately. Practitioners citing F1 from `bnmetrics` should be explicit
