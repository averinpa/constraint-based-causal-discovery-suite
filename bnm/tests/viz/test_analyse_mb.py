"""Smoke tests for `bnm.analyse_mb`."""
...
def test_one_panel_per_metric() -> None:
    ...
    descriptive metric."""
    fig = bnm.analyse_mb(asia_8())
    assert len(fig.data) == len(bnm.DESCRIPTIVE_METRIC_NAMES)


def test_specific_metrics_subset() -> None:
    fig = bnm.analyse_mb(asia_8(), descriptive=["n_edges", "n_colliders", "n_root_nodes"])
    assert len(fig.data) == 3


def test_unknown_metric_raises() -> None:
    with pytest.raises(bnm.BNMInputError, match="unknown descriptive metric"):
        bnm.analyse_mb(chain_3(), descriptive=["not-a-metric"])
...
def test_empty_graph_raises() -> None:
    ...
    g = bnm.to_graphlike(np.zeros((0, 0), dtype=np.int8))
    with pytest.raises(bnm.BNMInputError, match="no variables"):
        bnm.analyse_mb(g)
...
def test_chain_n_edges_distribution_hand_computed() -> None:
    ...
    """
    fig = bnm.analyse_mb(chain_3(), descriptive=["n_edges"])
    trace = fig.data[0]
...
def test_collider_n_colliders_distribution() -> None:
    ...
    fig = bnm.analyse_mb(collider_3(), descriptive=["n_colliders"])
    trace = fig.data[0]
    pairs = dict(zip(trace.x, trace.y, strict=True))
    assert pairs == {"1": 3}
...
def test_save_html(tmp_path) -> None:
    ...
    bnm.analyse_mb(asia_8(), save=out)
    assert out.exists()
...
def test_works_on_index_only_graph() -> None:
    ...
    fig = bnm.analyse_mb(g, descriptive=["n_edges"])
...
def test_custom_title() -> None:
    fig = bnm.analyse_mb(asia_8(), title="Custom title")
...
def test_n_isolated_nodes_in_chain() -> None:
    ...
    should be {0: 3}."""
    fig = bnm.analyse_mb(chain_3(), descriptive=["n_isolated_nodes"])
    trace = fig.data[0]
    pairs = dict(zip(trace.x, trace.y, strict=True))
    assert pairs == {"0": 3}
