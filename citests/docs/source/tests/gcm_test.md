...
**Empty conditioning set**: when `condition_set` is empty (or `None`), the wrapper substitutes a single constant column $Z = 0$ before passing to pycomets. This means `GCM(data)(X, Y, [])` tests $X \perp Y$ marginally via residualisation against a constant, not via the no-conditioning path. The other 16 tests in citests pass an empty conditioning set through unchanged. Same behavior applies to :doc:`/tests/wgcm_test` and :doc:`/tests/pcm_test`.

## Code Example

```python
import numpy as np
from citests.tests import GCM
...
For a full list of parameters, see the API documentation: :class:`citests.tests.ml_based_tests.GCM`.
